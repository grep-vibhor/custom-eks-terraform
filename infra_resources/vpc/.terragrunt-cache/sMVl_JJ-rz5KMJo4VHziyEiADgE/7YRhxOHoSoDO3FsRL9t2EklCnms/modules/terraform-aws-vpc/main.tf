locals {
  max_subnet_length = max(
    ...
    length(var.gateway_subnets),
    length(var.pod_subnets),
  )
...
resource "aws_route_table" "database" {
  ...
  tags = merge(
    {
      "Name" = var.single_nat_gateway || var.create_database_internet_gateway_route ? "${var.name}-${var.database_subnet_suffix}-rt" : format(
...
resource "aws_route_table" "gateway" {
  count = var.create_vpc && length(var.gateway_subnets) > 0 ? 1 : 0

  vpc_id = local.vpc_id

  tags = merge(
    {
      "Name" = format("%s-${var.gateway_subnet_suffix}", var.name)
    },
    var.tags,
    var.gateway_route_table_tags,
  )
}

resource "aws_route" "gateway_internet_gateway" {
  count = var.create_vpc && var.create_igw && length(var.gateway_subnets) > 0 ? 1 : 0
...
resource "aws_route" "gateway_internet_gateway_ipv6" {
  count = var.create_vpc && var.create_igw && var.enable_ipv6 && length(var.gateway_subnets) > 0 ? 1 : 0
...
resource "aws_route_table" "pod" {
  count = var.create_vpc && local.max_subnet_length > 0 && length(var.pod_subnets) > 0 ? local.nat_gateway_count : 0

  vpc_id = local.vpc_id

  tags = merge(
    {
      "Name" = var.single_nat_gateway ? "${var.name}-${var.pod_subnet_suffix}-rt" : format(
        "%s-${var.pod_subnet_suffix}-%s",
        var.name,
        element(var.azs, count.index),
      )
    },
    var.tags,
    var.pod_route_table_tags,
  )
}
...
resource "aws_subnet" "gateway" {
  count = var.create_vpc && length(var.gateway_subnets) > 0 ? length(var.gateway_subnets) : 0

  vpc_id                          = local.vpc_id
  cidr_block                      = element(concat(var.gateway_subnets, [""]), count.index)
  availability_zone               = length(regexall("^[a-z]{2}-", element(var.azs, count.index))) > 0 ? element(var.azs, count.index) : null
  availability_zone_id            = length(regexall("^[a-z]{2}-", element(var.azs, count.index))) == 0 ? element(var.azs, count.index) : null
  assign_ipv6_address_on_creation = var.gateway_subnet_assign_ipv6_address_on_creation == null ? var.assign_ipv6_address_on_creation : var.gateway_subnet_assign_ipv6_address_on_creation

  ipv6_cidr_block = var.enable_ipv6 && length(var.gateway_subnet_ipv6_prefixes) > 0 ? cidrsubnet(aws_vpc.this[0].ipv6_cidr_block, 8, var.gateway_subnet_ipv6_prefixes[count.index]) : null

  tags = merge(
    {
      "Name" = format(
        "%s-${var.gateway_subnet_suffix}-%s",
        var.name,
        element(var.azs, count.index),
      )
    },
    var.tags,
    var.gateway_subnet_tags,
  )
}
...
resource "aws_subnet" "pod" {
  count = var.create_vpc && length(var.pod_subnets) > 0 ? length(var.pod_subnets) : 0

  vpc_id                          = local.vpc_id
  cidr_block                      = element(concat(var.pod_subnets, [""]), count.index)
  availability_zone               = length(regexall("^[a-z]{2}-", element(var.azs, count.index))) > 0 ? element(var.azs, count.index) : null
  availability_zone_id            = length(regexall("^[a-z]{2}-", element(var.azs, count.index))) == 0 ? element(var.azs, count.index) : null
  assign_ipv6_address_on_creation = var.pod_subnet_assign_ipv6_address_on_creation == null ? var.assign_ipv6_address_on_creation : var.pod_subnet_assign_ipv6_address_on_creation

  ipv6_cidr_block = var.enable_ipv6 && length(var.pod_subnet_ipv6_prefixes) > 0 ? cidrsubnet(aws_vpc.this[0].ipv6_cidr_block, 8, var.pod_subnet_ipv6_prefixes[count.index]) : null
...
################################################################################
# Gateway Network ACLs
################################################################################

resource "aws_network_acl" "gateway" {
  count = var.create_vpc && var.gateway_dedicated_network_acl && length(var.gateway_subnets) > 0 ? 1 : 0
  ...
  tags = merge(
    ...
    var.tags,
    var.gateway_acl_tags,
  )
}

resource "aws_network_acl_rule" "gateway_inbound" {
  count = var.create_vpc && var.gateway_dedicated_network_acl && length(var.gateway_subnets) > 0 ? length(var.gateway_inbound_acl_rules) : 0

  network_acl_id = aws_network_acl.gateway[0].id

  egress          = false
  rule_number     = var.gateway_inbound_acl_rules[count.index]["rule_number"]
  rule_action     = var.gateway_inbound_acl_rules[count.index]["rule_action"]
  from_port       = lookup(var.gateway_inbound_acl_rules[count.index], "from_port", null)
  to_port         = lookup(var.gateway_inbound_acl_rules[count.index], "to_port", null)
  icmp_code       = lookup(var.gateway_inbound_acl_rules[count.index], "icmp_code", null)
  icmp_type       = lookup(var.gateway_inbound_acl_rules[count.index], "icmp_type", null)
  protocol        = var.gateway_inbound_acl_rules[count.index]["protocol"]
  cidr_block      = lookup(var.gateway_inbound_acl_rules[count.index], "cidr_block", null)
  ipv6_cidr_block = lookup(var.gateway_inbound_acl_rules[count.index], "ipv6_cidr_block", null)
}

resource "aws_network_acl_rule" "gateway_outbound" {
  count = var.create_vpc && var.gateway_dedicated_network_acl && length(var.gateway_subnets) > 0 ? length(var.gateway_outbound_acl_rules) : 0

  network_acl_id = aws_network_acl.gateway[0].id

  egress          = true
  rule_number     = var.gateway_outbound_acl_rules[count.index]["rule_number"]
  rule_action     = var.gateway_outbound_acl_rules[count.index]["rule_action"]
  from_port       = lookup(var.gateway_outbound_acl_rules[count.index], "from_port", null)
  to_port         = lookup(var.gateway_outbound_acl_rules[count.index], "to_port", null)
  icmp_code       = lookup(var.gateway_outbound_acl_rules[count.index], "icmp_code", null)
  icmp_type       = lookup(var.gateway_outbound_acl_rules[count.index], "icmp_type", null)
  protocol        = var.gateway_outbound_acl_rules[count.index]["protocol"]
  cidr_block      = lookup(var.gateway_outbound_acl_rules[count.index], "cidr_block", null)
  ipv6_cidr_block = lookup(var.gateway_outbound_acl_rules[count.index], "ipv6_cidr_block", null)
}
...
resource "aws_network_acl" "pod" {
  count = var.create_vpc && var.pod_dedicated_network_acl && length(var.pod_subnets) > 0 ? 1 : 0
  ...
  tags = merge(
    {
      "Name" = format("%s-${var.pod_subnet_suffix}", var.name)
    },
    var.tags,
    var.pod_acl_tags,
  )
...
resource "aws_network_acl_rule" "pod_inbound" {
  count = var.create_vpc && var.pod_dedicated_network_acl && length(var.pod_subnets) > 0 ? length(var.pod_inbound_acl_rules) : 0

  network_acl_id = aws_network_acl.pod[0].id

  egress          = false
  rule_number     = var.pod_inbound_acl_rules[count.index]["rule_number"]
  rule_action     = var.pod_inbound_acl_rules[count.index]["rule_action"]
  from_port       = lookup(var.pod_inbound_acl_rules[count.index], "from_port", null)
  to_port         = lookup(var.pod_inbound_acl_rules[count.index], "to_port", null)
  icmp_code       = lookup(var.pod_inbound_acl_rules[count.index], "icmp_code", null)
  icmp_type       = lookup(var.pod_inbound_acl_rules[count.index], "icmp_type", null)
  protocol        = var.pod_inbound_acl_rules[count.index]["protocol"]
  cidr_block      = lookup(var.pod_inbound_acl_rules[count.index], "cidr_block", null)
  ipv6_cidr_block = lookup(var.pod_inbound_acl_rules[count.index], "ipv6_cidr_block", null)
}

resource "aws_network_acl_rule" "pod_outbound" {
  count = var.create_vpc && var.pod_dedicated_network_acl && length(var.pod_subnets) > 0 ? length(var.pod_outbound_acl_rules) : 0

  network_acl_id = aws_network_acl.pod[0].id

  egress          = true
  rule_number     = var.pod_outbound_acl_rules[count.index]["rule_number"]
  rule_action     = var.pod_outbound_acl_rules[count.index]["rule_action"]
  from_port       = lookup(var.pod_outbound_acl_rules[count.index], "from_port", null)
  to_port         = lookup(var.pod_outbound_acl_rules[count.index], "to_port", null)
  icmp_code       = lookup(var.pod_outbound_acl_rules[count.index], "icmp_code", null)
  icmp_type       = lookup(var.pod_outbound_acl_rules[count.index], "icmp_type", null)
  protocol        = var.pod_outbound_acl_rules[count.index]["protocol"]
  cidr_block      = lookup(var.pod_outbound_acl_rules[count.index], "cidr_block", null)
  ipv6_cidr_block = lookup(var.pod_outbound_acl_rules[count.index], "ipv6_cidr_block", null)
}
...
resource "aws_route" "pod_nat_gateway" {
  count = var.create_vpc && var.enable_nat_gateway && length(var.pod_subnets) > 0 ? local.nat_gateway_count : 0

  route_table_id         = element(aws_route_table.pod.*.id, count.index)
  destination_cidr_block = "0.0.0.0/0"
  nat_gateway_id         = element(aws_nat_gateway.this.*.id, count.index)
...
resource "aws_route" "pod_ipv6_egress" {
  count = var.create_vpc && var.create_egress_only_igw && var.enable_ipv6 && length(var.pod_subnets) > 0 ? length(var.pod_subnets) : 0
...
resource "aws_route_table_association" "gateway" {
  ...
  subnet_id      = element(aws_subnet.gateway.*.id, count.index)
  route_table_id = aws_route_table.gateway[0].id
}
...
resource "aws_route_table_association" "pod" {
  ...
  subnet_id = element(aws_subnet.pod.*.id, count.index)
  route_table_id = element(
