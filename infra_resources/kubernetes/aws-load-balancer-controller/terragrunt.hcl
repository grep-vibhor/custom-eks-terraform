...
terraform {
  source = "${local.global_var.inputs.module_source_path}/kubernetes/aws-load-balancer-controller"
}
...
inputs = {
  ...
  image_repository = "602401143452.dkr.ecr.${local.global_var.inputs.region}.amazonaws.com/amazon/aws-load-balancer-controller"
  enableBackendSecurityGroup = false # enables shared security group for backend traffic # https://kubernetes-sigs.github.io/aws-load-balancer-controller/v2.5/deploy/security_groups/
}